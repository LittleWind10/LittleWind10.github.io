function latestProjectList() {
	const sortedProjects = projects
		.filter((project) => project.projectDate && project.projectId)
	...
	latestProjects.forEach((project) => {
		...
		const typeLatestContainer = latestCard.querySelector(
			'.type-latest-container'
		);
...
function projectList() {
	...
	function updateProjectCards() {
		...
		const filteredProjects = projects
			.filter((project) => {
				const projectTypes = project.projectCategory
					.split(', ')
					.map((type) => type.trim().toLowerCase());
				const typeMatch =
					selectedType === 'all' || projectTypes.includes(selectedType);
				return typeMatch;
			})
			.sort((a, b) => {
		...
		filteredProjects.forEach((project) => {
			if (project.projectId) {
				...
				const typeProjectContainer = projectCard.querySelector(
